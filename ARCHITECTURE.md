...
2. **WebSocket upgrade** — extracts client IP (respects Cloudflare headers)
3. **Protocol version check** — rejects clients with wrong `Sec-WebSocket-Protocol`
4. **Admission handshake** — `perform_admission()` → `admit()`
5. **Registration** — insert into router, spawn message loop
6. **Cleanup** — remove from router on disconnect, decrement gauges
