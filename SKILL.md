...
On Linux, the installer starts the daemon automatically via systemd. On macOS, start it manually (see Step 2).
...
If it fails with "Failed to connect to daemon" or similar:
...
On macOS (or if not using systemd):
```bash
pkill -f "arpc start" 2>/dev/null; sleep 1; arpc start &
```

Detect which to use: `if command -v systemctl &>/dev/null && systemctl is-active arpc &>/dev/null; then systemctl restart arpc; else pkill -f 'arpc start' 2>/dev/null; sleep 1; arpc start &; fi`
...
systemctl stop arpc 2>/dev/null  # Linux systemd
